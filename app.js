...
function findLesson(id) {
  for (const m of course.modules) {
    const raw = rawLessons(m).find((l) => l.id === id);
    if (raw && isModuleQuiz(raw)) {
      const fallback = [...lessonsOf(m)].reverse().find((l) => l.type === "lesson") || lessonsOf(m)[0];
      return { module: m, lesson: fallback, skippedQuiz: true };
    }
...
function rawLessons(mod) {
  return (mod.lessons || []).filter((l) => l && l.id);
}

function isModuleQuiz(lesson) {
  return Boolean(lesson && lesson.type === "quiz" && !lesson.final);
}

function lessonsOf(mod) {
  return rawLessons(mod).filter((l) => !isModuleQuiz(l));
}
...
function route() {
  ...
  if (path === "lesson" && id) {
    ...
    if (opened.skippedQuiz) {
      currentId = opened.lesson.id;
      currentStage = "watch";
      if (location.hash !== "#/lesson/" + opened.lesson.id) location.hash = "/lesson/" + opened.lesson.id;
      view = hasAccess() ? "lesson" : "login";
    } else if (opened.module && opened.module.hidden) {
      currentId = "m1-l1";
      currentStage = "watch";
    ...
    <div class="site">
      ...
      <section class="strip" id="how">
        <div class="inner">
          ...
          <p><b>4 модуля программы</b><span>Лекции, практика и задание с Кирой</span></p>
        </div>
      </section>
...
function lockedHtml(reason, module, lesson) {
  ...
  let text = "Материалы открываются по порядку: видео, конспект и задание с Кирой.";
  let action = "";
  if (reason === "need-pay") {
    ...
    text = prev
      ? `Для перехода к этой теме завершите видео и задание предыдущего модуля «${prev.title}».`
...
function homeworkPrompt(lesson) {
  const task = lesson.homework || {};
  const prompt = String(task.prompt || "Опишите одним абзацем, что для вас сейчас самое важное в этой теме.").trim();
...
function lessonKiraThread(lesson) {
  const lessonId = typeof lesson === "string" ? lesson : lesson.id;
  const saved = Array.isArray(lessonChat[lessonId]) ? lessonChat[lessonId] : [];
  const item = typeof lesson === "string" ? findLesson(lesson).lesson : lesson;
  const q = { id: "q-" + lessonId, name: "Кира AI", me: false, text: homeworkPrompt(item) };
  if (saved.length) {
    return saved.some((m) => m.id === q.id) ? saved : [q].concat(saved);
  }
  const hw = homework[lessonId];
  const out = [q];
  if (hw && hw.text) {
    out.push({ id: "a-" + lessonId, name: (user && user.name) || "Вы", me: true, text: hw.text });
  }
  if (hw && hw.review) {
    out.push({ id: "rev-" + lessonId, name: "Кира AI", me: false, text: reviewPlain(hw.review) });
  }
  return out;
}
...
function guideHtml() {
  ...
  return `
    ...
    <section class="guide-feat">
      ...
      <div>
        <p class="section-label">Ваш маршрут</p>
        <h3>Четыре недели, чтобы снова стать опорой себе</h3>
        <p class="lede">Сначала вы увидите механизмы созависимости в своей жизни. Затем научитесь разделять ответственность, выдерживать чувство вины и выстраивать близость без потери себя.</p>
        <ul class="guide-list">
    ...
    <section class="guide-feat is-rev">
      ...
      <div>
        <p class="section-label">Ваш персональный AI-ассистент</p>
        <h3>Кира помогает перенести знания из урока в реальную жизнь</h3>
        <p class="lede">Иногда всё понятно в теории — до первого сложного разговора. В такой момент можно обратиться к Кире и спокойно разобрать происходящее.</p>
        <div class="guide-facts">
          <div><b>Знает программу курса</b><p>Кира опирается на лекции Василия Шурова, практики и 14 схем созависимых отношений.</p></div>
          <div><b>Разбирает вашу ситуацию</b><p>Опишите конкретный эпизод, сложный разговор или сомнение — Кира поможет увидеть действующую схему и отделить факты от тревоги.</p></div>
          <div><b>Помогает сделать следующий шаг</b><p>Подскажет, как сформулировать границу, подготовиться к разговору и выполнить задание без самообвинения.</p></div>
        </div>
        <div class="row"><a class="btn ghost" href="#/kira">познакомиться с Кирой</a></div>
      </div>
    </section>
    ...
    <section class="guide-feat">
      ...
      <div>
        <p class="section-label">Атлас курса</p>
        <h3>14 схем, которые делают невидимое понятным</h3>
        <p class="lede">Созависимые отношения часто кажутся хаосом из тревоги, надежды и повторяющихся конфликтов. Схемы помогают увидеть не отдельные ссоры, а устойчивый механизм.</p>
        <div class="guide-facts">
          <div><b>Узнать знакомый сценарий</b><p>Слияние, спасательство, треугольник Карпмана, чувство вины, размытые границы — с примерами из повседневной жизни.</p></div>
          <div><b>Понять, почему он повторяется</b><p>Каждая схема показывает, что поддерживает привычный круг и в какой точке можно начать действовать иначе.</p></div>
          <div><b>Возвращаться, когда трудно</b><p>Используйте атлас во время уроков, перед сложным разговором или вместе с Кирой. Это навигатор для самостоятельной практики, а не диагностика.</p></div>
        </div>
        <div class="row"><a class="btn ghost" href="#/atlas">открыть атлас схем</a></div>
      </div>
    </section>

    <section class="section">
      <div class="section-label">Перед началом обучения</div>
      <h3>Оформите данные участника</h3>
      <div class="guide-facts">
        <div><b>Лицензированная программа</b><p>Школа работает на основании образовательной лицензии №1035-01255-50/01675078 от 27.12.2024, выданной Министерством образования Московской области.</p></div>
        <div><b>Конфиденциальность</b><p>Ваши ответы, задания и прогресс относятся к личному профилю участника и используются только для организации обучения.</p></div>
        <div><b>Налоговый вычет</b><p>После оплаты обучения можно оформить социальный налоговый вычет в порядке, предусмотренном законодательством РФ.</p></div>
      </div>
      <div class="tool-box cert-box" id="certBox">
        <h4>Нужен ли вам сертификат?</h4>
        <p class="cert-lead">Выберите один вариант и сохраните данные. Этот шаг нужен для оформления участия и открытия материалов курса.</p>
        ...
        <form id="certForm" class="cert-form">
          <label>ФИО / Full name*<input name="fio" type="text" autocomplete="name" value="${esc(cert.fio || "")}" placeholder="Как в документе" /></label>
          <label>Номер документа*<input name="passport" type="text" autocomplete="off" value="${esc(cert.passport || "")}" placeholder="Как в паспорте или ID — любой страны" /></label>
          <label>Кем и когда выдан*<input name="issued" type="text" value="${esc(cert.issued || "")}" placeholder="Орган и дата / Authority & date" /></label>
          ...
          <label>Адрес*<input name="address" type="text" autocomplete="street-address" value="${esc(cert.address || "")}" placeholder="Город, страна / City, country" /></label>
          <p class="fine">Подойдёт паспорт или ID любой страны — формат номера любой. Код подразделения заполняют только с паспортом РФ. Если сертификат не нужен — в обязательных полях можно поставить прочерк и сохранить форму.</p>
...
function bindLessonKira(lesson) {
  ...
  lessonChat[lesson.id] = lessonKiraThread(lesson);
  const grow = () => {
    if (!box) return;
  ...
  if (form) {
    form.onsubmit = (e) => {
      ...
      if (box) box.value = "";
      grow();
      const done = Boolean(homework[lesson.id] && homework[lesson.id].text);
      if (!done) {
        homework[lesson.id] = { text, review: null };
        save(LS.hw, homework);
        progress[lesson.id] = true;
        save(LS.progress, progress);
        const mine = { id: "a-" + lesson.id, name: (user && user.name) || "Вы", me: true, text };
        lessonChat[lesson.id] = (lessonChat[lesson.id] || lessonKiraThread(lesson)).concat(mine);
        save(LS.lessonChat, lessonChat);
        if (log) log.insertAdjacentHTML("beforeend", kiraMsgHtml(mine));
        if (log) log.scrollTop = log.scrollHeight;
        afterAnswer();
        syncProfile();
        startHomeworkStream(lesson, text);
        return;
      }
